      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div
            ...
            <motion.button onClick={() => setIsMenuOpen(false)}
              className="absolute top-4 right-4" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
              <X />
            </motion.button>
            <nav className="mt-8">
              <ul className="space-y-4">
                {[
                  ...
                  <li key={label}>
                    <a href="#" className="text-gray-300 hover:text-blue-500 transition-colors flex items-center">
                      <Icon className="mr-2" size={16} />{label}
                    </a>
                  </li>
                ))}
              </ul>
    ...
    <motion.div {...fadeInUp} className="border-b border-gray-700">
      <div className="container mx-auto px-4">
        <ul className="flex space-x-8 overflow-x-auto">
          {["Stocks", "Mutual Funds", "ETFs", "Options", "Futures"].map((tab) => (
            <motion.li key={tab}
              className={`py-2 cursor-pointer whitespace-nowrap ${activeTab === tab
    ...
    const interval = setInterval(() => {
      setMarketData((prevData) =>
        prevData.map((index) => {
          ...
          return { ...index, value: index.value + change, change, percentChange };
        })
      );
    }, 1000);
        ...
        <motion.div key={index.name}
          ...
          <div className="flex items-center space-x-2">
            <span className="text-lg text-white">
              {index.value.toLocaleString("en-IN", { style: "currency", currency: "INR" })}
            </span>
            <motion.span key={index.change} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
  ...
  return (
    <motion.div
      ...
      <div className="flex items-center justify-between">
        ...
        <motion.span key={change} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          className={`text-sm flex items-center ${change >= 0 ? "text-green-500" : "text-red-500"}`}>
...
const MostBought = () => (
  <motion.div {...fadeInUp} className="my-8">
    <div className="flex justify-between items-center mb-4">
      ...
      <motion.a href="#" className="text-blue-500 text-sm hover:underline flex items-center"
        whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
...
const ProductsAndTools = () => {
  ...
  const handleClick = (name: string) => {
    if (name === "Charts") router.push("/charts");
            ...
            <AnimatePresence>
              {expandedCompany === company.name && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
                  ...
                  <p>You can add more details, charts, or any other relevant data.</p>
